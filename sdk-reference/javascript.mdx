...
## update_document vs update_document_fields

These two methods behave very differently. Understanding 
the difference is critical to avoid accidentally losing data.

### updateDocument - Replaces entire document
WARNING: This replaces ALL fields. Any field not 
included will be permanently deleted.

// Original document:
// { name: "John", age: 25, city: "Lagos", role: "admin" }
...
// age, city and role are GONE!

### updateDocumentFields - Updates only specified fields
This is the safe option. Only updates fields you provide.
All other fields remain untouched.

// Original document:
// { name: "John", age: 25, city: "Lagos", role: "admin" }

await db.updateDocumentFields("users", "user-123", {
  name: "John Doe"
});

// Result: { name: "John Doe", age: 25, city: "Lagos", role: "admin" }
// Only name was updated, everything else preserved!

Use updateDocument when you want to completely 
replace a document with new data.

Use updateDocumentFields when you want to partially 
update specific fields only. This is the safer and 
more common choice.
...
Use onAuthEvent to react to authentication state changes 
across your app. Register callbacks once when your app 
initializes.
...
db.auth.onAuthEvent({
  ...
  onAuthStateChange: (user, token) => {
    // Called on initAuth() - use to restore session
    if (user) {
...
// Remove all callbacks when done (e.g. component unmount)
db.auth.clearAuthCallbacks();
```

Note: onAuthEvent is framework-agnostic. Use it to 
update React state, Vue stores, or Svelte stores 
without polling. Register once on app initialization.
