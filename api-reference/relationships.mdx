...
const posts = await fetch('https://api.cocobase.cc/collections/posts?populate=author&populate=comments');

// Fetch user profile with followers and following populated
const profile = await fetch('https://api.cocobase.cc/auth-collections/user?populate=followers_ids&populate=following_ids');
```
