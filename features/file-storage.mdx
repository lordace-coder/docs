...
<Tab title="Dart">
```dart
// Note: File uploads during registration are not yet supported in the Flutter SDK.
await db.register(
  email: 'john@example.com',
...
<Tab title="Dart">
```dart
// Note: File uploads during user update use db.updateUser() in the Flutter SDK.
// Update user avatar
await db.updateUser(
