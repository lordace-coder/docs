...
```javascript
import { Cocobase, CollectionEvent } from "cocobase";

const db = new Cocobase({ apiKey: "your-api-key" });

// Watch collection
const connection = db.realtime.collection(
  "users",
  (event: CollectionEvent) => {
    switch (event.type) {
      ...
      case "create":
        ...
        break;
      case "update":
        console.log("✏️ Updated:", event.data.id);
        break;
      case "delete":
        console.log("🗑️ Deleted:", event.data.id);
        break;
    }
  },
  {
...
<Tab title="JavaScript">
```javascript
const connection = db.realtime.collection('users',
  (event) => {
...
class ResilientWatcher {
  ...
  watch(handler) {
    try {
      this.connection = this.db.realtime.collection(
        this.collection,
        handler
      );
    } catch (error) {
...
useEffect(() => {
  const connection = db.realtime.collection('posts', handler);
...
db.realtime.collection('posts', handler, {
